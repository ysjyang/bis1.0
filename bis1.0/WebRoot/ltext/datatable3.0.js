...
Ext.lt.datatable35=function(ds){
	...
	// 缺省列实现             
	var _defaultcolumns={
		seq:{fixi:1,isDef:true,name:'_locationposition',alias:'序号',width:40,minwidth:40,fn:function(l,c,rs,v){
				return '<center>'+(l+1)+'</center>';
			}},
		checkbox:{isDef:true,name:'check',isDisabled:false,alias:'<input type="checkbox">',width:22,minwidth:22,style:'text-align:center',
			//初始化时需要改变颜色
			fn:function(l,c,rs,v){
				return '<input type="checkbox" '+(_defaultcolumns.checkbox.isDisabled?'disabled':'')+'  '+(v==this._checkvalue?'checked':'')+' style="margin-top:3px;">'
			},
			_checkvalue:1,
			_alias:'',
			...
			onclick:function(table,el,l,c,d){
				var popt=document.getElementById(table.id+'_l'+l+'_c'+c+(_clock>c?'_g':'_d'));
				if(popt==null){
					return ;
				}
				var opt=popt.firstChild;
				if(!!opt.disabled){
					return;
				}
				
					if(this.beforeclick!=null){
						if(this.beforeclick(table,el,l,c,d)==false){
							//datatable1001_l0_c0_d
							var popt=document.getElementById(table.id+'_l'+l+'_c'+c+(_clock>c?'_g':'_d'));
							if(popt==null){
								return ;
							}
					...
					if(d[this.name] != null) {
						Ext.lt.message.send("datatable","onChecked",[table,el,l,c,d]);
					} else {
						Ext.lt.message.send("datatable","unChecked",[table,el,l,c,d]);
					}
						//开始判断是否选中第一个。
					...
					//1.子集任意选中父级显示选中
					//从当前节点网上找数据一直到level=1
					//同时列数据节点按照级次的每级都相同的。
					
					//2.父级选中子集全部被选中
					//父级选中子集全部选中，采用方式：
					//从当点数据开始向下循环一直到level<=当前的值
					
					//3.子集全部选中父级为选中
					//这个比较恶心人。
					//首先循环 获取level=1且等于当前选择规则的。
					// 之后获取level=2 ... 
					//判断每级节点是否都被选中，如果都被选中则选中上级依次递归。
					//6.子集选中父级成半勾选状态
					//如果没有被都选中则设定为半勾选状态。并且设置相关字段。
					...
					// 以上注释是不是出现在了不应该出现的位置 —— wengyuedong 2015.09.24
			...
			onheadclick:function(table,el,c,rs){
				...
				if(rs.size()==0)check=false;
				rs.each(function(rs){
					check?rs[f]=v:rs[f]=null});
				this.alias='<input type="checkbox" '+(check?'checked':'')+'>';dt.reflash()
				if (check) {
					Ext.lt.message.send("datatable","onAllSelected",[dt]);
				} else {
					Ext.lt.message.send("datatable","onAllunSelected",[dt]);
				}
		...
		radio:{isDef:true,name:'check',isDisabled:false,alias:'&nbsp;',width:22,minwidth:22,_checkvalue:1,_lastchecked:null,
			//初始化时需要改变颜色
			fn:function(l,c,rs,v){ if(v==this._checkvalue){this._dt._checkradio=l;this._lastchecked=rs}return '<input type="radio" '+(_defaultcolumns.radio.isDisabled?'disabled':'')+'   name="'+_tableid+'_radio_'+this.name+'" '+(v==this._checkvalue?'checked':'')+'>'},
			setColumnName:function(n){this.name=n;return this},
			setCheckValue:function(v){_checkvalue=v;return this;},
			onclick:function(table,el,l,c,d){
				//if(this.beforeclick!=null){this.beforeclick(table,el,l,c,d)}
				var opt = document.getElementById(table.id+'_l'+l+"_c"+colindex+(_clock>colindex?'_g':'_d'));

				if(opt&&(!!opt.firstChil.disabled)){
					return;
				}
	...
	for(var i=0,l=_events.length;i<l;i++){
		...
		Ext.lt.bindEvent(document.body,_events[i],function(event){
//			console.log(event.type);
			_defaultEventFunction(event);
			});
	}
	//特殊监听
/*
...
*/
		...
		if(_head.contains(el) || _cross.contains(el)){
			...
			while(_head.contains(el) || _cross.contains(el)){
				...
				// 如果鼠标点击在表头TD区域时出发表头事件
					...
					var colindex=el.colindex;
					if(colindex==null&&el.getAttribute!=null){
	...
	function _doscroll(sl,st){
		...
		new function(){
			...
			if(_showclockdiv.fixscrollTop!=0){
				...
				if(_showclockdiv.parentElement.scrollTop!=0) _showclockdiv.parentElement.scrollTop=0;
			}
	...
	function _dofillData(sl,st){
		...
		//alert("t="+t+";st="+st+";_tbodyHeight="+_tbodyHeight+";_tagdiv.scrollTop="+_tagdiv.scrollTop);
		// 如果起始位置没变，则不需要重新生成数据
		...
		_showdatadiv.innerHTML='';
		if(_showdatadiv.scrollTop!=0)_showdatadiv.scrollTop=0;
		if(_showdatadiv.scrollLeft!=0)_showdatadiv.scrollLeft=0;
		_showclockdiv.innerHTML='';
		if(_showclockdiv.scrollTop!=0)_showclockdiv.scrollTop=0;
		if(_showclockdiv.scrollLeft!=0)_showclockdiv.scrollLeft=0;
	...
	function _fillShowDataDivHTML(shwodiv,startline,startcolumn,fixleft){
		...
		if(firstline.length>0){
			// 计算填充列数
			var fillwidth=function(){return _showdatadiv.offsetWidth+(showcl<firstline.length?firstline.item(showcl).offsetWidth:0);}();
			var _column;

			for(var sc=startcolumn;sc<colsize;sc=nextColumnId(sc)){
				if(showcl>=linesize) break;
				_column=firstline.item(showcl++);
				
				_fillColumn++;
				l+=parseInt(Ext.lt.HTML.getRealStyle(_column).width,10)+Ext.lt.HTML.getBorderSet(_column).width;
				
				if(_column.firstChild!=null){
					var w=_column.firstChild.offsetWidth;
					groupwidth+=isNaN(w)?0:w;
				}
				if(l>(fillwidth)) break;		
			}
			var left=_column.offsetLeft;
			if(left>0) groupwidth=left;
	...
	function _divscroll(flag){
		...
		//_initFillLine  _allowlineheight
		if(_tagdiv.style.overflowY=='hidden'){
			_tagdiv.scrollTop=0;
		}
		// 记录滚动条位置，如果没动则不处理
		...
		// 强制重新生成数据
		if(flag==true){
			_laststartline=null,_laststartcolumn=null,_lastScrollLeft=st,_lastScrollTop=st;
		}
		_doscroll(sl,st)
		_showmouselightcell()
	...
	function _checkStringFormat(c){
		...
		// 处理日期数据显示格式
	function _checkBooleanFormat(c){
		c.format={};
		c._formatValue=function(value){
		...
		if(!c.fn){
			...
			c.fn=function(i,j,rs,value){return 	'<span><input type="checkbox" disabled=false '+(value==1?'checked=true':'')+' /></span>';}	
		}
		c._fn=function(i,j,rs){
			var value=rs[this.name];
			// 转换为十进制浮点数
			...
			if(null==al && 'undefined'==al){
				al='<input type="checkbox" disabled=false />'
			}
			return al+'';
		}
	}
	...
	function _filteStringColumn(filterwindow,div,_rs,_rs_bak){
								...
								for(var i=0,l=_rs_bak.length;i<l;i++){
									...
									if(ldata['v_'+v]==null){
										ldata.push(v);
										ldata['v_'+v]=true;
										ldata['o_'+v]={v:v,t:col._formatValue(v)};
									}
								}
								ldata._sort(true);
	...
	function _addCol(c){
		...
		if(c._formatValue==null) c._formatValue=function(v){return v}
		if(!c.fn){
		...
		if(c.gfn==null){
			c.gfn=function(rs,value){
				var v=this._formatValue(value),ginfo=_rs.getgroupinfo(rs[_groupbycolumn.name]);
				if(ginfo==null) 
					return '找不到该组数据';
				if(v==null || v=='') v='空'
...
//		for(var i=0,l=_columns_all.length;i<l;i++) if(_columns_all[i].display) _columns.push(_columns_all[i]);
		var colnames = eval(decodeURIComponent(Ext.lt.cache.getData('dtcolumn_' + _tableid)));
		if (colnames != null) {
			for (var i = 0, len = colnames.length; i < len; i++){
				_columns.push(_columns_all['col:'+colnames[i]]);
			}
		}else{
			for(var i=0,l=_columns_all.length;i<l;i++) if(_columns_all[i].display) _columns.push(_columns_all[i]);
		}
	...
	function _buildCSS(){
		...
		if(_table.style==null){
			if (document.createStyleSheet){
				...
				_table.style=document.getElementById(_tableid+'_css').sheet;
		...
		if(Ext.lt.ischrome){
			csstxt.push(" .",_tableid," .h table{margin-left:-1px}");
			csstxt.push(" .",_tableid," .ch table{margin-left:-1px}");
		}
	  ...
	  _table.style.setCssText(csstxt.join(''));
		_table.columnstyle=[]
		// 将列样式对象分离出来		
		for(var j=0, colsize=_columns.length,rulesize=_table.style.rules.length;j<colsize;j++){
			var styleobj=_table.style.rules.item(rulesize-j-1);
	...
	function _buildColHead(){
		...
		for(var i=0;i<_columns.length;i++){
			var c=_columns[i];
			if(c.isDef&&c.name=='check'&&c.alias!=""){
				 _columns[0]._dt.checkbox_object=cols[i];
			}
		}
		if(_coldrag){
			_buildColDrag(cols)	
	...
	function _buildColDrag(cols){
		...
		Ext.lt.bindEvent(_head,'onselectstart',function(en){
			...
			function dragHead(en){
				...
				if(tablebody.contains(en.srcElement)){
					...
					while(_head.contains(el)){
						...
						if(endcolindex){
							endcolindex=parseInt(el.getAttribute('colindex'),10)
						}
			...
			function enddragHead(en){
				...
				enddragHead.unbindEvent(document,'onmousemove');
				enddragHead.unbindEvent(document,'onmouseup');
			...
			Ext.lt.bindEvent(document,'onmousemove',dragHead);
			Ext.lt.bindEvent(document,'onmouseup',dragHead);
	...
	function _buildColWidth(hcols){
		for(var sindex=0,end=hcols.length;sindex<end;sindex++){
			...
			col.setAttribute('colindex',colindex)
			...
			col.onmousedown=function(){
				...
				if(linediv==null){
					...
					linediv.style.cssText='position:absolute;width:1px;height:5000px;top:0px;left:0px;border-left:1px dotted #666666;';
...
//					table.setObj(o);
					...
					// 修改列宽样式单
					var colindex=ss.colindex;
					if(!colindex==null){
						colindex=col.getAttribute('colindex')
					}
					_buildColWidthCSS(colindex,ss.offsetWidth+wc);
					// 保存新宽度 lp 20130309
					if (_userwid) {
	...
	function _sortData(colname,asc){
		...
		switch(config.datatype){
			case 'S':fv= Ext.lt.util.fnbind(config._formatValue,config);break;
			case 'D':fv= Ext.lt.util.fnbind(config._formatValue,config);break;
	...
	function _buildHeadMenu(hcols,sindex,_col){
		...
		for(var end=hcols.length;sindex<end;sindex++){
			...
			col.oncontextmenu =function(){
				var _c=this;
				var datas=[];
				var colindex=_c.colindex
				if(!colindex){
					colindex=_c.getAttribute('colindex');
				}
				var _menucol=table.getCol(colindex);
				if(_menucol.isDef){
					if(_allowclock){
				...
				//		datas.push({id:4,name:'锁定',code:'cs1',pid:0,ico:'/ltext/images/datatable/menulock.gif'});
					...
					if(_allowcustom){
						datas.push({id:9,name:'显示',code:'cs1',pid:0,click:function(){
							var configwindowdiv=document.createElement('DIV');
							configwindowdiv.style.cssText='width:400px;height:500px;display:block;overflow:hidden;background-color:white';
							document.body.appendChild(configwindowdiv);
							var configwindow=new Ext.lt.window({title:'<font style="font-weight:bold;color:#012A3A;font-size:14px">表格配置</font>',fitmode:'content',pop:true,mark:true,className:'windpblue',bodystyle:'padding:0px;overflow:hidden'});
							configwindow.draw(configwindowdiv);
							configwindowdiv.innerHTML='<div style="width:100%;height:460px;display:block;background-color:white"></div><div class="dt_configbg" style="width:100%;height:40px;display:block;text-align:center;"><button style="width:80px;height:25px;margin:5px">重置</button><button style="width:80px;height:25px;margin:5px">确定</button><button style="width:80px;height:25px;margin:5px">取消</button></div>'
							...
							for(var i=0,l=_columns.length;i<l;i++){
								tempcol = _columns[i]; 
								if(tempcol.name!="_locationposition" && tempcol.name!="check"){
									ordernum ++;
									var width = tempcol.width;
									if(width ==0){
										width = tempcol.headobject.offsetWidth;
										tempcol.offsetWidth = width;
									}
									columnconfigrs.addData({'name':tempcol.name,'alias':tempcol.alias,'width':width,'check':'1','ordernum':ordernum});
								}
							}
							for(var i=0,l=_columns_all.length;i<l;i++){
								tempcol=_columns_all[i];
								if(tempcol.name!="_locationposition" && tempcol.name!="check"){
									var show = true;
									for(var j=0,jl=_columns.length;j<jl;j++){
										if(tempcol.name == _columns[j].name){
											show = false;
											break;
										}
									}
									if(show){
										ordernum ++;
										columnconfigrs.addData({'name':tempcol.name,'alias':tempcol.alias,'width':tempcol.width,'ordernum':ordernum});
									}
								}
							}
							var configtable=new Ext.lt.editdatatable(columnconfigrs);
							var _checkcol = configtable.columns['checkbox'];
							_checkcol.alias ="显示";
							_checkcol.width = 40;
							configtable.setCols([
								configtable.columns['seq'],
								_checkcol,
								/*{name:'name',alias:'列编码',width:100},*/
								{name:'alias',alias:'列名称',width:'auto',width:140,edit:true},
								{name:'ordernum',alias:'顺序',datatype:'I',width:60,edit:true,size:2,
								format:{unit:1,dotname:0,qfw:false},oneditend:function(table1,el,l,c,d){
									Ext.lt.ui.alert("调整顺序！",function(){});
									//window.setTimeout(function(){
										table1.getRecordset().sort("ordernum",true,function(v){return parseInt(v,10)});
										var datas = table1.getRecordset().toArray();
										for(var i=0,l=datas.length;i<l;i++){
											datas[i]['ordernum'] = i+1;
										}
									//},1);
								}},
							...
							btn.item(0).onclick=function(){
								Ext.lt.cache.removeData('dtcolumn_'+_tableid,"null");
								for(var i=0,l=_columns_all.length;i<l;i++) {
									var tempcol = _columns_all[i];
									Ext.lt.cache.removeData(_tableid+tempcol.name);
									if (tempcol.oldwidth != null) tempcol.width = tempcol.oldwidth;
								}
								_table.redraw();
								configwindow.close();
							}
							...
							btn.item(1).onclick=function(){
								var tempcol,colname=[];
								_columns = [];
								if(_columns_all['col:'+'_locationposition']){
									colname.push('_locationposition');
									_columns.push(_columns_all['col:'+'_locationposition']);
								}
								if(_columns_all['col:'+'check']){
									colname.push('check');
									_columns.push(_columns_all['col:'+'check']);
								}
								columnconfigrs.each(function(d){
									if(d['check']!=1) return;
									tempcol=_columns_all['col:'+d['name']];
									tempcol.alias = d.alias;
									colname.push(d['name']);
									_columns.push(tempcol);
									if(d['width'] && d['width']!=tempcol.offsetWidth && d['width']!=tempcol.oldwidth){
										Ext.lt.cache.setData(_tableid+d['name'], d['width']);
									}
									
								})
								if(colname.length>2){
									// 想cookie中写入数据
									Ext.lt.cache.setData('dtcolumn_'+_tableid,Object.toJSON(colname));
									_table.redraw();
									configwindow.close();
								}else{
									alert("至少选择一列显示！");
							...
							// 取消按钮
							btn.item(2).onclick=function(){
								configwindow.close();
							}
							
						},icoclass:'btn_config35'});
					}
				}
				...
				d.style.left=(colp.left-(_clock<colindex?_head.scrollLeft:0)+1)+'px';
	...
	// 表头菜单
	function _buildBodyMenu(btns){
		_showdatadiv.oncontextmenu =function(){
			if(this.menu==null){
				this.menu=Ext.lt.Popupmenu({
					data:btns,
					field:{id:'id',name:'name',code:'code',sid:'pid'}
				});
				var d=document.getElementById('datatablemenu');
				if(d==null){
					var d=document.createElement('div');
					document.body.appendChild(d);
					d.id=table.id+'datatablebodymenu';
				}
				this.menu.draw(d);
				this.menudiv=d;
			}
			this.menudiv.style.left=event.clientX
			this.menudiv.style.top=event.clientY
			this.menu.show();
			return false;
		}
	}
	// 表头菜单
	var _showEndMenuBtn=null;
	var _showEndMenuDiv=null;
	function _buildEndMenu(btns){
		_showEndMenuBtn=btns;
	}
	function _showEndMenu(l){
		//如果不是滚动的事件就不理他
		if(event.type!="mouseover")return;
		if(!_showEndMenuBtn)return;
		var showspan=document.getElementById(_tableid+'_l'+l+'_d');
		if(showspan==null){
			showspan=document.getElementById(_tableid+'_l'+l+'_g');
		}
		//计算行号好了。给个div开始显示
		if(!this._doShowEndMenu){
			var sv=document.createElement('span');
			sv.className='showendmenu';
			sv.id='showendmenu';
			_tagdiv.parentElement.insertAdjacentElement('beforeBegin',sv);
			this._doShowEndMenu=sv;
		}
		if(this._doShowEndMenu.innerHTML==''){
			var _endMenuHtml=['<table cellpadding="0" cellspacing="0" border="0"><tr><td nowrap="nowrap"><p>']
			for(var i=0;i<_showEndMenuBtn.length;i++){
				_endMenuHtml.push(_showEndMenuBtn[i].name);
			}
			_endMenuHtml.push['</p></td></tr></table>'];
			this._doShowEndMenu.innerHTML=_endMenuHtml.join("");
		}
		//把行号放进去了;
		var d=_rs.getData(l);
		var ln=showspan.lastChild;
		var x=ln.offsetLeft+ln.clientWidth;
		//debugger;
		var y=ln.offsetTop;
		showspan.appendChild(this._doShowEndMenu);
		this._doShowEndMenu.style.left=x-this._doShowEndMenu.offsetWidth;
		//this._doShowEndMenu.style.top=y;
	}
	function _closeEndMenu(){
	...
	function _buildInitTable(){
		...
		//var h=_tbodyHeight*(_rs.size()+(_rs.getgroupsize()>0?10:0))+_head.offsetHeight+20,divp=0,divh=1000000
		//解决最下面的行的数据显视不全的问题。205-3-31 modify by zzg
		var h=_tbodyHeight*(_rs.size()+(_rs.getgroupsize()>0?10:0))+_head.offsetHeight+_tbodyHeight*2,divp=0,divh=1000000
		fixhdiv=_div;
		do{
		...
		if(correction!=null){
			...
			fixhdiv.style.marginBottom=(fixhdiv.parentNode.parentNode.clientHeight-_head.offsetHeight)/3+'px';
		}
	}
	...
	// 清除填充标志重新绘制表格
	function _reflash(){
		var st=_tagdiv.scrollTop;
		_showdatadiv.show=false
		_buildInitTable();
		var allheight=_rs.size()*_tbodyHeight;
		if(allheight>=st){
			_tagdiv.scrollTop=st;
		}else{
			_tagdiv.scrollTop=allheight;
		}
		_fixDivWidth();
		_divscroll(true);
	}
			...
			if(id.length>tableidlength){ 
				...
				do{
					...
					if(sv!=data[_columns[c].name]+''){
						...
						cell.setAttribute('standvalue',data[_columns[c].name]);
					}
					cell=cell.nextSibling;
				}
				while(cell!=null)
	...
	_table=table
		...
		table.destory=function(){
			...
			table.style.cssText='';
				if(Ext.lt.ischrome){
					table.style.removeRule();
				}else{
					table.style.owningElement.removeNode(true);
				}
			}
			document.body.removeChild(filterwindow);
		...
		table.resize=function(w,h){
			if(!_drawed) return;
//			if(_resizew==w && _resizeh==h) return;
		...
		table.reflash=function(l,c){
			...
			if(l=='viewdata'){
				...
				if(_defaultcolumns.checkbox.id!=null){
					//重置复选框
					var checked=this.getRecordset().query({check:1}).length==this.getRecordset().size()&&this.getRecordset().size()>0;
					if(_defaultcolumns.checkbox.display){
						var colindex=_defaultcolumns.checkbox.colindex
						if(colindex==null&&_defaultcolumns.checkbox.getAttribute!=null){
							colindex=_defaultcolumns.checkbox.getAttribute('colindex')
						}
						var tnode = document.getElementById(this.id+"_h_"+colindex).firstChild;
						tnode.checked=checked;
						if(!!!_defaultcolumns.checkbox.isDisabled){
...
//						var pos=Ext.lt.HTML.positionedOffset(_tagdiv,_tagdiv.parentNode,false);
		...
		table.setRecordset=function(rs){
			...
			_rs=rs
			_rs.bindTable(table);
			if(_columns[0]._dt.checkbox_object!=null&&_columns[0]._dt.checkbox_object.firstChild){
				var _firstChid=_columns[0]._dt.checkbox_object.firstChild;
				if(_firstChid&&_firstChid.checked){					
					_firstChid.checked=_rs.query({check:1})>0
				}
			}
		}
...
//鼠标拖动行
	...
	table.swapColumn=function(colid,toid){
		...
		if(!srcColId){
			srcColId=srcCol.getAttribute('colindex');
		}
		if(!toColId){
			toColId=toCol.getAttribute('colindex');
		}
		_columns_all.splice(srcColId,1,toCol);
		_columns_all.splice(toColId,1,srcCol);
		srcCol.colindex=toColId;
		toCol.colindex=srcColId;
		//srcCol.setAttribute('colindex',toColId);
		//toCol.setAttribute('colindex',srcColId);
	...
	table.treelistcolumn=function(config){
		...
		var _inited=false;
		cmp._rs_bak=null;
		var _nullvalue=config.nullvalue==null?null:config.nullvalue;
		cmp.dtfn_getRecordSet;
		cmp.clear=function(){
			cmp._rs_bak=null;
		}
		cmp.init=function(){
			...
			if(this._dt.getRecordSet().toArray().length>0){
			// 自动处理数据级次
			if(autolevel){
			...
			// 按照初始显示级次，过滤数据
			if(cmp.treeaction){
				cmp._rs_bak=this._dt.getRecordSet().toArray();
				var _rs=this._dt.getRecordSet();				
				var datas_bak=cmp._rs_bak,datas=[],i=0,newrs=new Ext.lt.recordset({columns:_rs.getColNames(),'datas':[]});
			...
			if(null==cmp.dtfn_getRecordSet){
				cmp.dtfn_getRecordSet=table.getRecordSet;
			}
			this._dt.getRecordSet=function(){
				if(cmp._rs_bak==null){return table.getRecordset();}
				var rs = new Ext.lt.recordset({columns:_rs.getColNames(),'datas':[]})
				rs.addData(cmp._rs_bak);
				return rs;
				}
			}
		...
		cmp.fn=function(i,j,rs,value){
			...
			var colindex=_columns[level].colindex;
			if(!colindex){
				colindex=_columns[level].getAttribute('colindex')
			}
			out.push(_columns[level]._fn(colindex,j,rs));
		...
		cmp.onclick=function(table,el,l,c,d){
			...
			if(d[statcolumn]=='close'){
				...
				for(var loop=cmp._rs_bak.length;i<loop;i++){
					var data=cmp._rs_bak[i];
				...
				d[statcolumn]='open';
				cmp.dtfn_getRecordSet().addData(instdata,l);
			}
			else if(d[statcolumn]=='open'){
				var f=true;
				d[statcolumn]='close';
				cmp.dtfn_getRecordSet().delData(function(data){
	...
	table.contextMenu=function(btns){
		if(!lt.isArray(btns)){btns=[btns]}
		_buildBodyMenu(btns)
	}
	table.endColMenu=function(btns){
		if(!lt.isArray(btns)){btns=[btns]}
		_buildEndMenu(btns)
	}
	// 表格排序方法
