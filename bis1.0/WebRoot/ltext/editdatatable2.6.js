...
Ext.lt.editdatatable=Ext.lt.createComponent(function(rs){
		...
		_editor.onkeydown=function(){
			...
			if(_editor.col.datatype!='S'){
				...
				if(window.getSelection!=null){
					...
					length=seled.createRange().text.length;
				}else{
				...
				if(_editor.col.numinpformat!=null){
					if(event.keyCode==189||event.keyCode==190||event.keyCode==110||event.keyCode==109){return true;}
					var vlength=v.length;
					if(v.indexOf('-')==0){
						vlength--;
					}
					var int=_editor.col.numinpformat.int;
					var dec=_editor.col.numinpformat.dec;
					var input_int=v.length;
					var input_dec=0;
					if(index!=-1){
						input_int=index;
						input_dec=vlength-index-1;
					}
					if(input_int>=int||input_dec>=dec){
						return false;
					}
				}else{
	...
	table.setCols=function(c){
		...
		for(var n=0;n<c.length;n++){
			...
			if(col.edit==true){
				...
				if(col.datatype=='B'){
					if(col.fn == null){
						col.fn=function(i,j,rs,value){return 	'<span><input type="checkbox" '+(value==1?'checked=true':'')+' /></span>';}	
					}
	...
	function setUpdataValue(d,cname){
		...
		if(updateData[d._locationposition]==null){
			...
			//updateData[d._locationposition][cname]=d[cname];
		}
