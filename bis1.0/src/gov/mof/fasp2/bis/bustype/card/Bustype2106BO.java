package gov.mof.fasp2.bis.bustype.card;
...
public class Bustype2106BO extends BustypeCommonBO{
	...
	public Map readDatas(String maintablecode, String subtablecode,
        ...
        if (datas != null && datas.size() > 0) {
            addSubData2(datas,subtablecode);
        }
	...
	public Map replySlipDatas(String maintablecode, String subtablecode,
        ...
        if (datas.size() > 0) {
            ...
            CommonDAO dao = (CommonDAO)ServiceFactory.getBean("bis.common.CommonDAO");
            String insql = this.parseInSql2("billid", datas);
            ...
            if(listmap.size()!=num){
            	...
            	for(int i=0;i<listmap.size();i++){
                	...
                	sql = "select * from "+maintablecode+" t where nvl(t.bisreadstatus,0)=1 and billid='"+guid+"'";
...
//            	throw new AppException("0018");
            ...
            String sql = "update "+maintablecode+" t set t.bisreplyslipstatus=1,t.bisreplysliptime='" 
            				+bisreplysliptime+"' where nvl(t.bisreadstatus,0)=1 and "+insql;
            dao.update(sql);
        }
        rt.put("result", "SUCCESS");
        return rt;
