...
public class Bustype1109BO extends BustypeCommonBO {
	...
	public Map cancelSendDatas(String maintablecode, String subtablecode,
        ...
        //取消的数据主单billid拼上bisbankcode
        for (int i = 0; i < listmap.size(); i++) {
        	mainmap = (Map)listmap.get(i);
        	mainmap.put("billid", mainmap.get("bisbankcode")+"-"+mainmap.get("billid"));
        	sublist1 = (List)mainmap.get("sublist");
        	for (int j = 0; j < sublist1.size(); j++) {
        		submap = (Map)sublist1.get(j);
        		submap.put("mainguid",submap.get("bisbankcode")+"-"+submap.get("mainguid"));
			}
		}
        //查询要取消的数据是否存在,并且锁定这些记录.
        String sql ="select * from "+cancelmaintablecode+" where "+this.parseInSql2("billid", listmap) +" for update";
        ...
        String subsql ="select * from "+cancelsubtablecode+" where "+this.parseInSql2("mainguid", sublist) +" for update";
    	if(dao.queryForList(subsql)!=null){
    		lsub.addAll(dao.queryForList(subsql));
    	}
        //要取消的数据是否都存在
        if(l.size()!=listmap.size()){
            String missingguid = this.getMissingDatas(listmap, l, "billid");
        ...
        for(int i=0;i<l.size();i++){
            ...
            if(bisreadstatus == 1){
                if(s.length()==0){
                    s.append(m.get("billid"));
                }else{
                    s.append(",").append(m.get("billid"));
                }
            }
        }
        // 对子表的数据进行操作
        ...
        dao.update(sql);
        sql ="delete from "+cancelmaintablecode+" where "+this.parseInSql2("billid", listmap);
