...
/**
 * @ClassName: Bustype1019BO
 * @Description: Description of this class
 * @author <a href="mailto:zhongyi@szlongtu.com">钟毅</a>于 2016-5-17 上午09:23:37
 */

public class Bustype1019BO extends BustypeCommonBO {
    ...
    public Map cancelSendDatas(String maintablecode, String subtablecode, String cancelmaintablecode,
        ...
        String sql ="select * from "+cancelmaintablecode+" where "+this.parseInSql2("planguid", listmap) +" for update";
        List l = dao.queryForList(sql);
        
        //要取消的数据是否都存在
        if(l.size()!=listmap.size()){
            String missingguid = this.getMissingDatas(listmap, l, "planguid");
        ...
        for(int i=0;i<l.size();i++){
            ...
            if(bisreadstatus == 1){
                if(s.length()==0){
                    s.append(m.get("planguid"));
                }else{
                    s.append(",").append(m.get("planguid"));
                }
            }
        }
        if(s.length()>0){
        ...
        sql ="delete from "+cancelmaintablecode+" where "+this.parseInSql2("planguid", listmap);
