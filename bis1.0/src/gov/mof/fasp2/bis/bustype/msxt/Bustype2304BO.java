...
public class Bustype2304BO extends BustypeCommonBO {
    ...
    public Map sendDatas(String maintablecode, String subtablecode, List listmap) throws AppException {
        ...
        if(refundList.size()>0){
        	//数据重复校验
        	CheckData.comCheckRepeat(refundList, maintablecode.replace("1305", "1306"), "guid" , this.parseInSql2("guid", refundList));
            //保存数据
            try {
                dao.saveAll(refundList, maintablecode.replace("1305", "1306"));
            } catch (Exception e) {
                e.printStackTrace();
