...
public class Bustype1102BO extends BustypeCommonBO {
	...
	public Map cancelSendDatas(String maintablecode, String subtablecode,
        ...
        String sql ="select * from " + cancelmaintablecode+" where " + this.parseInSql4("cardno", "agencycode", listmap) 
        				+ " for update";
        List l = dao.queryForList(sql);
        
        //要取消的数据是否都存在
        if(l.size()!=listmap.size()){
            String missingguid = this.getMissingDatas(listmap, l, "cardno");
        ...
        for(int i=0;i<l.size();i++){
            ...
            if(bisreadstatus == 1){
                if(s.length()==0){
                    s.append(m.get("cardno"));
                }else{
                    s.append(",").append(m.get("cardno"));
                }
            }
        }
        if(s.length()>0){
        ...
        sql ="delete from " + cancelmaintablecode + " where " + this.parseInSql4("cardno", "agencycode", listmap);
