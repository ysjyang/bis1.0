...
/**
 * @ClassName: Bustype2011BO
 * @Description: Description of this class
 * @author <a href="mailto:huchuan@szlongtu.com">胡川</a>于 2016-5-23 下午7:50:47
 */

public class Bustype2026BO extends BustypeCommonBO {
    ...
    public Map readDatas(String maintablecode, String subtablecode, int maxdatasnum,String bankcode,String bustype)
        ...
        String sql = "select " + this.getQueryTableColumns(maintablecode) + " from " + maintablecode
                + " t where nvl(t.bisreadstatus,0)=0";
        if (maxdatasnum > 0) {
        ...
        if (datas != null && datas.size() > 0) {
            addSubData3(datas,subtablecode);
        }
        ...
        if (datas.size() > 0) {
            ...
            sql = "update " + maintablecode + " t set t.bisreadstatus=1,t.bisreadtime='" + bisreadtime + "' where "
                    + this.parseInSql4("guid","biscanceltime", datas);
            dao.update(sql);
        }
    ...
    public Map readUnReplySlipDatas(String maintablecode, String subtablecode, int maxdatasnum,String bankcode,
        ...
        // 查询 数据并锁定数据 如果maxdatasnum有值 则查询时带上rownum
        String sql = "select " + this.getQueryTableColumns(maintablecode) + " from " + maintablecode
                + " t where nvl(t.bisreadstatus,0)=1 and nvl(t.bisreplyslipstatus,0)=0";
        if (maxdatasnum > 0) {
            sql += " and rownum<=" + maxdatasnum;
        }
        sql += " for update";
    ...
    public Map replySlipDatas(String maintablecode, String subtablecode, List listmap,String bustype) 
        ...
        if (datas.size() > 0) {
            ...
            CommonDAO dao = (CommonDAO)ServiceFactory.getBean("bis.common.CommonDAO");
            String insql = this.parseInSql5("biscreatetime","biscanceltime","guid", datas);
            String checksql = "select count(*) num from "+maintablecode+" t where nvl(t.bisreadstatus,0)=1 and "
            		+"nvl(t.bisreplyslipstatus,0)=0 and "+insql;
