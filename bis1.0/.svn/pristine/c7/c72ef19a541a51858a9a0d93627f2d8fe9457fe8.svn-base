...
public class Bustype2102BO extends BustypeCommonBO{
	...
	public Map readDatas(String maintablecode, String subtablecode,
        ...
        if(datas.size()>0){
            ...
            sql = "update "+maintablecode+" t set t.bisreadstatus=1,t.bisreadtime='"+bisreadtime+"' where "+this.parseInSql2("cardno", datas);
            dao.update(sql);
        }
	...
	public Map replySlipDatas(String maintablecode, String subtablecode,
        ...
        if(datas.size()>0){
            ...
            CommonDAO dao = (CommonDAO)ServiceFactory.getBean("bis.common.CommonDAO");
            String insql = this.parseInSql2("cardno", datas);
            ...
            if(listmap.size()!=num){
            	...
            	for(int i=0;i<listmap.size();i++){
                	...
                	sql = "select * from "+maintablecode+" t where nvl(t.bisreadstatus,0)=1 and cardno='"+cardno+"'";
                	list = dao.queryForList(sql);
                	if(list.size()==0){
                		if(i==listmap.size()-1){
                			errormsg += cardno+"是无效数据！";
                		}else{
                			errormsg += cardno+",";
                		}
                	}
